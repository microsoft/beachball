# Change Log - @microsoft/beachball-action-should-release

<!-- This log was last generated on Sat, 22 Aug 2026 00:26:19 GMT and should not be manually modified. -->

<!-- Start content -->

## 3.1.4

Sat, 22 Aug 2026 00:26:19 GMT

### Patches

- Update brace-expansion to 5.0.9 (elcraig@microsoft.com)

## 3.1.3

Thu, 13 Aug 2026 03:44:19 GMT

### Patches

- Update undici dependency (elcraig@microsoft.com)
- Update brace-expansion and minimatch dependencies (elcraig@microsoft.com)
- Use `node:` imports (elcraig@microsoft.com)

## 3.1.2

Wed, 29 Jul 2026 10:04:32 GMT

### Patches

- Update dependencies (elcraig@microsoft.com)
- Update dependency @actions/glob to ^0.7.0 (elcraig@microsoft.com)

## 3.1.1

Thu, 25 Jun 2026 01:55:16 GMT

### Patches

- Update dependencies (elcraig@microsoft.com)
- Move actions to beachball repo (elcraig@microsoft.com)
- Bump undici from 6.26.0 to 6.27.0 (elcraig@microsoft.com)
