...
describe('selectReleases', () => {
  ...
  it('filters by a case-insensitive substring of the tag', () => {
    ...
    expect(selectReleases(releases, options({ filter: 'APP' })).map(r => r.tag_name)).toEqual(['app_v2.0.0']);
  });
  ...
  it('filters by a /regex/ when the value is wrapped in slashes', () => {
    ...
    expect(selectReleases(releases, options({ filter: '/^v2\\./' })).map(r => r.tag_name)).toEqual([
      'v2.1.0',
      'v2.0.0',
    ]);
  });
  ...
  it('supports regex flags such as case-insensitivity', () => {
    ...
    expect(selectReleases(releases, options({ filter: '/^release-/i' })).map(r => r.tag_name)).toEqual([
      'Release-A',
      'release-b',
    ]);
  });
  ...
  it('throws a helpful error for an invalid /regex/ filter', () => {
    const releases = [makeRelease({ tag_name: 'v1.0.0' })];
    expect(() => selectReleases(releases, options({ filter: '/[/' }))).toThrow(/Invalid --filter regular expression/);
  });
  ...
  it('includes only releases published after the --since date', () => {
    ...
    expect(selectReleases(releases, options({ since: new Date('2024-02-01') })).map(r => r.tag_name)).toEqual([
      'v3.0.0',
    ]);
  });
});
