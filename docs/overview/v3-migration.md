...
- `registry` now uses whatever registry `npm` is configured to use.
  - **Temporary exception:** Due to unfinished features, if you're explicitly providing `NPM_TOKEN`/`BEACHBALL_NPM_TOKEN` or `--token`, you must explicitly provide `registry`.
