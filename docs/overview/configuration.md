...
| `publishRoot` | `string \| (({ packagePath, options }) => string)` | | repo | Run `npm publish`/`pack` from this directory instead of the package root. A string is resolved relative to each package root and must be relative. A function receives an object with the absolute package root and resolved beachball options, and may return a relative or absolute path. |
