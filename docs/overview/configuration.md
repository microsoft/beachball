...
| `registry` | `string` | | repo | Publish to this npm registry. If not set, resolved from `.npmrc` files using [`@npmcli/config`](https://www.npmjs.com/package/@npmcli/config) (with full environment variable substitution and credential support), falling back to `https://registry.npmjs.org/`. Auth credentials are also read from `.npmrc` when available. |
