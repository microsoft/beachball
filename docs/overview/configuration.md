...
| `packStyle` | `'sequential' \| 'layer'` | `'layer'` | repo | With `packToPath`, how to organize the tgz files. `'sequential'` uses numeric prefixes to ensure topological ordering. `'layer'` groups the packages into numbered subfolders based on dependency tree layers and writes a `versions.json` with the versions of the packed packages in each layer. |
