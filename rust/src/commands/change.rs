...
use crate::validation::validate::{validate, ValidateOptions, ValidationResult};
...
pub fn change(parsed: &ParsedOptions) -> Result<()> {
    ...
    let changed = changed_packages
        .unwrap_or_else(|| {
            get_changed_packages(options, &package_infos, &scoped_packages)
                .unwrap_or_default()
        });
    ...
    let dependent_change_type = options.dependent_change_type.unwrap_or(
