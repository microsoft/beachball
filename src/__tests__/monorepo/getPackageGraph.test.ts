...
describe('getPackageGraph', () => {
  ...
  /**
   * Ensure that both `getPackageGraph` and `getPancakes` return a valid ordering of packages,
  ...
  async function validateOrdering(
    ...
    const getPancakesOutput = getPancakes({
    ...
    expect(possibleSolutions).toContainEqual(getPancakesOutput);
